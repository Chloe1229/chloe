...
step6_items = {
    ...
    "s3_3": {
        ...
        "requirements": {
            ...
            "2": "2. 주사제, 점안제, 점이제로 원료약품의 종류가 동일하거나 허가된 첨가제가 다른 경우 : 「의약품의 품목허가·신고·심사 규정」 제27조제3항제2호에 따라 주사제는 보존제, 완충제, 항산화제, pH조절제(이미 허가·신고된 바있는 주사제에 사용된 pH조절제에 한함), 점안제 및 점이제는 보존제, 완충제, 등장화제, 점도조절제, pH조절제.",
            "3": "3. 흡입 전신마취제",
    ...
    for key in st.session_state.step6_targets:
        if key in step6_items:
            ...
            for sub_key, sub_text in block.get("subitems", {}).items():
                ...
                if key == "s4_5":
                    # Title 15: 하위항목 고정 "변경 있음"
                    st.session_state.step6_selections[full_key] = "변경 있음"
                    st.radio(f"{sub_text}", ["변경 있음"], index=0, key=full_key, disabled=True)

                elif key == "s4_2" and sub_key in ["12c1", "12c2"]:
                    # Title 12: 상호 연동
                    selected_value = st.radio(f"{sub_text}", ["변경 있음", "변경 없음"], key=full_key)
                    st.session_state.step6_selections[full_key] = selected_value

                    other_key = "12c2" if sub_key == "12c1" else "12c1"
                    mirror_full_key = f"{key}_sub_{other_key}"
                    st.session_state.step6_selections[mirror_full_key] = selected_value
                    st.radio(
                        f"{block['subitems'][other_key]}",
                        ["변경 있음", "변경 없음"],
                        index=0 if selected_value == "변경 있음" else 1,
                        key=mirror_full_key,
                        disabled=True
                    )

                elif key == "s4_6" and sub_key in ["16a", "16b"]:
                    # Title 16: 상호 연동
                    selected_value = st.radio(f"{sub_text}", ["변경 있음", "변경 없음"], key=full_key)
                    st.session_state.step6_selections[full_key] = selected_value

                    other_key = "16b" if sub_key == "16a" else "16a"
                    mirror_full_key = f"{key}_sub_{other_key}"
                    st.session_state.step6_selections[mirror_full_key] = selected_value
                    st.radio(
                        f"{block['subitems'][other_key]}",
            ...
            for req_key, req_text in block.get("requirements", {}).items():
                ...
                selected_value = st.radio(f"{req_text}", ["충족", "미충족"], key=full_key)
                st.session_state.step6_selections[full_key] = selected_value
