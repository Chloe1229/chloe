...
if "step6_page_index" not in st.session_state:
    st.session_state.step6_page_index = 0

current_index = st.session_state.step6_page_index
step6_targets = [key for key in st.session_state.step6_targets if key in step6_items]
...
if st.session_state.step == 6:
    ...
    if current_index < total_pages:
        key = step6_targets[current_index]
        ...
        for sub_key, sub_text in block.get("subitems", {}).items():
            ...
            # 12c1/12c2, 16a/16b 자동 동기화
                ...
                st.session_state.step6_selections[other_key] = selected_value
            else:
                selected_value = st.radio(
                    sub_text,
                    ["변경 있음", "변경 없음"],
        ...
        for req_key, req_text in block.get("requirements", {}).items():
            ...
            if req_text.strip().startswith(f"{req_key}."):
                label = req_text
            else:
        ...
        with col1:
            if current_index > 0:
                st.button("이전단계로", on_click=lambda: st.session_state.update(step6_page_index=current_index - 1))

        with col2:
            if current_index < total_pages - 1:
                st.button("다음항목 선택하기", on_click=lambda: st.session_state.update(step6_page_index=current_index + 1))
            else:
                st.button("결과 확인하기", on_click=lambda: st.session_state.update(step=7))
